...
type Config struct {
	...
	Scope      []string //analyzed scope -> from user input: -path; not used now
...
type ResultWCtx struct {
	...
	TestEntries map[*ssa.Function]*cgnode // the cgnodes where the tests officially start
...
// GetLabelFor bz: user API: get the label for an object with nodeid id
func (r *ResultWCtx) GetLabelFor(id int) *Label {
	return r.a.labelFor(nodeid(id))
}
...
// bz: user API: return PointerWCtx for a ssa.Value used under context of []*callsite
//input: ssa.Value, []*callsite;
//output: PointerWCtx; this can be empty with nothing if we cannot match any
func (r *ResultWCtx) PointsToByContext(v ssa.Value, ctx []*callsite) PointerWCtx {
	...
	for _, pts := range ptss {
		if EqualCallSite(ctx, pts.GetMyContext()) {
			return pts
		}
	}
	if ptss == nil {
	...
	if r.DEBUG {
		fmt.Println(" ****  Pointer Analysis cannot match this ssa.Value: " + v.String() + " with this context: " + ctx[0].String() + " **** ") //panic
	}
	return PointerWCtx{a: nil}
}
...
func (r *ResultWCtx) PointsToByGo(v ssa.Value, goInstr *ssa.Go) PointerWCtx {
	...
	ptss = r.pointsToRegular(v) //return type: []PointerWCtx
	for _, pts := range ptss {
		if pts.MatchMyGoInstruction(goInstr, nil) {
			return pts
		}
	}
	if r.DEBUG {
...
// PointsToByID return base ptr nodeid + offset
func (r *ResultWCtx) PointsToByID(base PointerWCtx, offset int) PointerWCtx {
	nid := base.n + nodeid(offset)
	return PointerWCtx{a: r.a, n: nid, cgn: base.cgn}
}
...
// GetCalleeByID for once.Do makeclosure
func (r *ResultWCtx) GetCalleeByID(id int) *Node {
	nid := nodeid(id)
	n := r.a.nodes[nid]
	callee := n.obj.cgn
	return r.CallGraph.Nodes[callee]
}

// GetObjectByID debug
func (r *ResultWCtx) GetObjectByID(id int) *node {
	nid := nodeid(id)
	n := r.a.nodes[nid]
	return n
}

// GetPTSByPtrID get pts by a pointer nodeid
	...
	for _, pts := range ptss {
		if pts.cgn.fn == v.Parent() { //many same v (ssa.Value) from different functions, separate them
			if v.Parent().IsFromApp {
				if pts.MatchMyGoInstruction(goInstr, r.testMainCtx) {
					return pts
				}
			} else {
...
func (s PointsToSet) GetPTS() []int {
	if s.pts == nil {
		return make([]int, 0)
	}
	var space [50]int
	return s.pts.AppendTo(space[:0])
}
...
//TODO: this does not match parent context if callsite.length > 1 (k > 1)
func (p PointerWCtx) MatchMyGoInstruction(go_instr *ssa.Go, testMainCtx []*callsite) bool {
