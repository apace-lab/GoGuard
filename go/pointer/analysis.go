...
// bz: a summary of statistics and results
...
func (as *analyses) computeTotalCoverage() {
	covered := make(map[string]string)
	closure := make(map[string]string)
	other := make(map[string]string) //others can be lib, reflect, <root>

	for _, result := range as.main2ResultWCtx {
		allFns := result.a.allFns
		...
		fmt.Println("Coverage: ", (float64(len(covered))/float64(len(allFns)))*100, "%\t (#total: ",
			len(allFns), ", #compiled: ", len(result.a.cCmplFns),
...
type analysis struct {
	...
	online        bool                          //bz: whether a constraint is from genInvokeOnline() -> used for on-the-fly only
	go2dynfn      map[string]string             //bz: there are go invoked dynamic function calls, which requires a new context
	// we store signature (instr.String()@go_ref.String()) as key and value in map to generate new context for different go calls
...
func translateResult(_result *ResultWCtx, main *ssa.Package, as *analyses) *Result {
	result := &Result{
	...
	result.Queries = _result.Queries
	result.IndirectQueries = _result.IndirectQueries
	result.GlobalQueries = _result.GlobalQueries

	if as != nil {
		//upate
		as.main2Result[main] = result
		as.main2ResultWCtx[main] = _result
	}
...
func printConfig(config *Config) {
	...
	if len(config.Scope) == 0 {
		fmt.Println(" *** No Scope Pkgs **************** ")
	} else {
		fmt.Println(" *** Scope Pkgs ******************* ")
		if len(config.Scope) > 0 {
			for _, pkg := range config.Scope {
				fmt.Print(pkg + ", ")
			}
...
//AnalyzeMultiMains bz: user api, to analyze multiple mains sequentially
func AnalyzeMultiMains(config *Config) (results map[*ssa.Package]*Result, _results map[*ssa.Package]*ResultWCtx, err error) {
	if config.Mains == nil {
	...
	for i, main := range config.Mains { //analyze mains
		...
		translateResult(_result, main, as)
		elapse := time.Now().Sub(start)
		if as.maxTime < elapse {
			as.maxTime = elapse
		}
		if as.minTime > elapse {
			as.minTime = elapse
		}
		as.total = as.total + elapse.Milliseconds()
	...
	//bz: i want this...
	fmt.Println("Total: ", (time.Duration(as.total)*time.Millisecond).String()+".")
	fmt.Println("Max: ", as.maxTime.String()+".")
	fmt.Println("Min: ", as.minTime.String()+".")
	fmt.Println("Avg: ", float32(as.total)/float32(len(config.Mains))/float32(1000), "s.")

	if flags.DoCoverage { //bz: total coverage
		as.computeTotalCoverage()
	}

	return as.main2Result, as.main2ResultWCtx, nil
}
...
func Analyze(config *Config) (result *Result, err error) {
	...
	result = translateResult(_result, main, nil)
	return result, nil
}
...
func AnalyzeWCtx(config *Config, doPrintConfig bool, isMain bool) (*ResultWCtx, error) { //Result
	if config.Mains == nil {
		return nil, fmt.Errorf("no main/test packages to analyze (check $GOROOT/$GOPATH)")
	}
	defer func() {
	...
	a := &analysis{
		...
		isMain:       isMain,
		go2dynfn:     make(map[string]string),
	}
	...
	if flags.DoCoverage && a.allFns == nil {
		a.collectFnsWScope()
	...
	for _, cgns := range fns {
		for _, cgn := range cgns {
			for val, id := range cgn.localval {
				...
				a.fillResultQueries(val, id, cgn)
			}
			...
			for obj, id := range cgn.localobj {
				...
				a.fillResultQueries(obj, id, cgn)
			}
		}
	}
	for val, id := range a.globalval {
		if id == 0 {
			continue
		}
		a.fillResultQueries(val, id, nil)
	}
	for obj, id := range a.globalobj {
		if id == 0 {
			continue
		}
		a.fillResultQueries(obj, id, nil)
	}

	return a.result, nil
}

// bz: fill in the result.Queries and result.IndirectQueries within the same analysis
func (a *analysis) fillResultQueries(val ssa.Value, id nodeid, cgn *cgnode) {
	if cgn == nil {
		ptr := PointerWCtx{a, id, nil}
		ptrs, ok := a.result.GlobalQueries[val]
		if !ok {
			// First time?  Create the canonical query node.
			ptrs = make([]PointerWCtx, 1)
			ptrs[0] = ptr
		...
		a.result.GlobalQueries[val] = ptrs
		return
	}

	t := val.Type()
	if CanPoint(t) { // direct
		ptr := PointerWCtx{a, id, cgn}
		ptrs, ok := a.result.Queries[val]
		if !ok {
			// First time?  Create the canonical query node.
			ptrs = make([]PointerWCtx, 1)
			ptrs[0] = ptr
		...
		a.result.Queries[val] = ptrs
	} else { //indirect
		ptr := PointerWCtx{a, id, cgn}
		ptrs, ok := a.result.IndirectQueries[val]
		if !ok {
			ptrs = make([]PointerWCtx, 1)
			ptrs[0] = ptr
		} else {
		...
		a.result.IndirectQueries[val] = ptrs
	}
}
...
func (a *analysis) collectFnsWScope() {
	a.allFns = make(map[string]string)
	a.cCmplFns = make(map[string]string)
...
func (a *analysis) computeCoverage() {
	...
	fmt.Println("\n#Coverage: ", (float64(len(covered))/float64(len(a.allFns)))*100, "%\t (#total: ", len(a.allFns), ", #compiled: ", len(a.cCmplFns),
