...
//bz: going to store (borrow them a pointer) a.localval (and a.localobj) here when calling genFunc() (gen.go)
//only copy if func is in analysis scope
func (n *cgnode) copyLocalMaps(a, b map[ssa.Value]nodeid) {
...
func (n *Node) GetCalleeByCommon(site_in *ssa.CallCommon) []*Node {
	ret := make([]*Node, 0)
	for _, edge := range n.Out {
		site := edge.Site.Common()
		if site == site_in {
			ret = append(ret, edge.Callee)
		}
	}
	return ret
}
...
// EqualCallSite bz: if two []*callsite, csa and csb, are the same
