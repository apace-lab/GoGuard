...
func (a *analysis) setValueNode(v ssa.Value, id nodeid, cgn *cgnode) {
	if cgn != nil { // bz: both a.localval and a.globalval; will be nullify at the end of genFunc()
		a.localval[v] = id
		if a.log != nil {
			fmt.Fprintf(a.log, "\tval[%s] = n%d  (%T) is localval\n", v.Name(), id, v)
		}
	} else {
		a.globalval[v] = id
		if a.log != nil {
			fmt.Fprintf(a.log, "\tval[%s] = n%d  (%T) is globalval\n", v.Name(), id, v)
		}
	}
	...
	////!!!! bz : this part is evil ... they may consider the performance issue,
...
func (a *analysis) withinScope(method string) bool {
	if a.config.LimitScope {
		...
		if strings.Contains(method, "command-line-arguments") || strings.HasPrefix(method, "main.") { //default scope or when running my tests in _test_main, _tests_callback
			return true
		} else {
	...
	if a.config.K > 0 {
		...
		if a.considerMyContext(fn.String()) {
			...
			if c, ok := instr.(*ssa.Call); ok {
				refs := *c.Referrers()
				for _, r := range refs {
					if go_ref, is_go := r.(*ssa.Go); is_go {
						if _, is_fn := go_ref.Call.Value.(*ssa.Call); is_fn {
							sig := instr.String() + "@" + go_ref.String()
							if _, exist := a.go2dynfn[sig]; !exist {
								go_call = go_ref // dynamic call invoked by go, need to update the context now
								break
							}
						}
					}
			...
			if okGo || okDefer || okCall || go_call != nil {     //bz: invoke a go routine --> detail check for different context-sensitivities; defer/call is similar
				if a.config.DEBUG { //debug
					fmt.Println("\tBUT ssa.GO/ssa.Defer/ssa.Call -- " + site.instr.String() + "   LET'S SEE.")
				}
				a.genStaticCallAfterMakeClosure(caller, instr, site, call, go_call, result) //bz: direct the handling outside and return
				return
			}
...
//both kcfa and origin, go_call can be nil
func (a *analysis) genStaticCallAfterMakeClosure(caller *cgnode, instr ssa.CallInstruction, site *callsite, call *ssa.CallCommon,
	go_call *ssa.Go, result nodeid) {
	fn := call.StaticCallee()
	...
	if ok { //exist closure add its new context, but no constraints
		...
		if go_call != nil {
			// dynamic fn call invoked by go
			site = &callsite{instr: go_call} // replace with a new context/callsite
		}
...
func (a *analysis) genFunc(cgn *cgnode) {
	...
	// bz: before setting to nil, let remember them for later use
	cgn.copyLocalMaps(a.localval, a.localobj)
